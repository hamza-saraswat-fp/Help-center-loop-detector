-- IAI-660 - Seed the `gap_check` prompt v1.0.4 (Gap check v1.0.4: headline sentence for the Slack card).
--
...
--
-- GENERATED from prompts/gap_check_v1_0_4.txt, which is the source of truth. Do not edit
-- the prompt text below by hand. Edit the text file and regenerate, keeping the
-- number this file already has:
--   node scripts/render-prompt-seed.js gap_check 1.0.4 anthropic/claude-sonnet-4.5 'Gap check v1.0.4: headline sentence for the Slack card' > migrations/NNNN_seed_gap_check_v1_0_4.sql
--
...
--
-- `version` is TEXT. It is the literal string '1.0.4', never a computed
-- max plus one.
--
-- Verify after applying (chars must be 8936):
...
Decide in this order. Do not skip ahead.

1. Is the question, as asked, answered by a sentence in one of the articles you were given? If yes, copy that sentence and its article path into `answered_by`. If no, set `answered_by` to null.
2. If `answered_by` is set, the verdict is `NOT_A_GAP`, or `NEEDS_EDIT` when that same article answers it but unclearly or incompletely. Whether the answering tool cited that article does not matter here; code handles uncited and hidden articles after you. An omission in a different article, an overview table, or a summary page never makes a covered question a gap.
3. If `answered_by` is null, compare the known truth to the articles claim by claim and choose `INCORRECT` when an article contradicts the truth, or `MISSING` when no article covers this.

The verdicts:

- `INCORRECT`: an article states something that contradicts the known truth. Quote the offending sentence in `says_now`.
- `MISSING`: no article covers this. You read the candidates in full and none of them answers the question. Only possible when `answered_by` is null.
- `NEEDS_EDIT`: an article covers the topic but not this specific point, or answers it unclearly. The reader would land on the right page and still leave without the answer.
- `NOT_A_GAP`: an article answers the question. Set `answered_by`.

Emit only those four. There are two further verdicts in this system, `UNFINDABLE` and `HIDDEN`, for articles that answer the question correctly but were never cited by the answering tool or are not published in navigation. Code decides those from `answered_by`, the claims, and the retrieval evidence, not you.
...
`headline` is one plain sentence that states the problem as a fact for a help center writer, not as a question, at most 140 characters, with no names or account identifiers. Examples: `The article says the card fee is typically 3%. A rep confirmed it is 4%.` or `The help center does not say that accounts synced with QuickBooks lose the Add Discount button.` When nobody has confirmed the right answer, say what is unclear: `The article does not explain what the Classic calendar option does.`
...
Reply with JSON and nothing else. No prose before it, no code fence around it. `answered_by.sentence` follows the same rule as claim sentences: copied character for character from the article text.

{
  ...
  "confidence": 0-100,
  "answered_by": { "article_path": "string", "sentence": "string" } | null,
  "claims": [
    {
      "claim": "string",
...
$p$,
  ...
  'Gap check v1.0.4: headline sentence for the Slack card'
);
