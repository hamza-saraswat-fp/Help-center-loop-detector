-- IAI-660 - Seed the `gap_check` prompt v1.0.3 (Gap check v1.0.3: ordered decision with answered_by; code enforces it).
--
...
--
-- GENERATED from prompts/gap_check_v1_0_3.txt, which is the source of truth. Do not edit
-- the prompt text below by hand. Edit the text file and regenerate, keeping the
-- number this file already has:
--   node scripts/render-prompt-seed.js gap_check 1.0.3 anthropic/claude-sonnet-4.5 'Gap check v1.0.3: ordered decision with answered_by; code enforces it' > migrations/NNNN_seed_gap_check_v1_0_3.sql
--
...
--
-- `version` is TEXT. It is the literal string '1.0.3', never a computed
-- max plus one.
--
-- Verify after applying (chars must be 8416):
...
$p$,
  ...
  'Gap check v1.0.3: ordered decision with answered_by; code enforces it'
);
