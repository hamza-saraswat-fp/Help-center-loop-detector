-- IAI-660 - Seed the `gap_check` prompt v1.0.2 (Gap check v1.0.2: a covered question is NOT_A_GAP even if another article omits it).
--
...
--
-- GENERATED from prompts/gap_check_v1_0_2.txt, which is the source of truth. Do not edit
-- the prompt text below by hand. Edit the text file and regenerate, keeping the
-- number this file already has:
--   node scripts/render-prompt-seed.js gap_check 1.0.2 anthropic/claude-sonnet-4.5 'Gap check v1.0.2: a covered question is NOT_A_GAP even if another article omits it' > migrations/NNNN_seed_gap_check_v1_0_2.sql
--
...
--
-- `version` is TEXT. It is the literal string '1.0.2', never a computed
-- max plus one.
--
-- Verify after applying (chars must be 7964):
...
- `MISSING`: no article covers this. You read the candidates in full and none of them answers the question. If any article in the packet answers the question, the verdict is `NOT_A_GAP`, even when another article or an overview table could also mention it. Never turn a covered question into `MISSING` because of an omission somewhere else. If the covering article itself is unclear or incomplete on the point, use `NEEDS_EDIT` on that article.
...
$p$,
  ...
  'Gap check v1.0.2: a covered question is NOT_A_GAP even if another article omits it'
);
