...
The service builds from the `Dockerfile` in this repo (Node 22 on Debian slim, plus `git`, which the sparse clone shells out to). Railway's default builder produced an image with Node 20 and no git, which is what took the first deploys down; the Dockerfile makes both explicit and reviewable. There is no `railway.json`: Railway no longer reads that file for services created after its Config-as-code deprecation (its API says so outright), and the replacement, Infrastructure as Code under `.railway/`, only applies through a manual `railway config apply`. The deploy settings therefore live on the service itself: Cron Schedule `0 * * * *`, Restart Policy Never, start command from the Dockerfile. Read them back with `railway status --json` or the service Settings page; if a deploy shows an empty cron or restart-on-failure, someone changed the service, not the repo. The first log line of every run is a preflight (`node <version> git <version or MISSING>`); if it says MISSING or below 22, stop there.
